...
* 12/25添加闹钟条目的启用与关闭功能,在list中添加int id和long time变量，实现闹钟的关闭和重新开启
*/
...
public class ClockDisplay extends Activity {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
	        ...
	        public void onItemClick(AdapterView<?> parent, View view, int position, //泛型,?表示不确定的java类型。  
	        	...
	        	if(Integer.parseInt(tempmap.get("onUse").toString()) == R.drawable.on_using_right){
	        		...
	        		ObjectPool.mAlarmHelper.closeAlarm(ClockID,"SmileAlarm", "time out!!");//删除闹钟的pendingintent
	        	}else{
	        		//开启闹钟
	...
	//长按菜单设置
	 ...
	 public boolean addList(int hour, int minute, int id, long time) {
		...
		//item.put("title", "闹钟"+(list.size()+1)); //标号在下面添加
		item.put("info", tmps);	
		int FlagToAdd =0;
		int cnt = 0;
		int order = -1;
